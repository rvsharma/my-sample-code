...
export default function DoctorsAndServices({
  goToSubStep,
  configData,
  setConfigData,
}: ownProps): JSX.Element {
  const addressableSpace = configData.modules.find(
    (x: any) => x.module === MODULE_NAMES.DOCTOR_SERVICES
  ...
  return (
    <>
      ...
      <div className='row member-signup-container shadow py-2 mt-4'>
        ...
        <Sections
          data={configData}
          module={MODULE_NAMES.DOCTOR_SERVICES}
          onModuleChange={changeConfigData}
        />
      </div>
      <div className='row member-signup-container shadow py-2 mt-4'>
        <div className='row member-signup-text pt-3 mb-4 lh-1'>
          <div className='col-12'>Enable & Disable Features</div>
        </div>
      ...
      <div className='row pt-5 pb-5 tab-button justify-content-center justify-content-sm-end'>
        ...
        <div className='col-auto'>
          <button
            ...
            onClick={() => goToSubStep(MODULE_NAMES.OUT_OF_POCKET)}>
            Next
          </button>
        </div>
      </div>
